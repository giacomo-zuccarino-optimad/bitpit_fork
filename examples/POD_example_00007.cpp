...
void run()
{
    ...
    */
    ...
    pod::PODField mode_1;
    // costuisco mode_1 con la build fields with coefficients
    ...
    // matrice di proiezione del modo così costruito
    ...
    // letto da file perchè la ricostruzione accetta solo i file letti
    ...
    // matrice dei coefficienti ricostruiti
    std::vector<std::vector<double>> mode1reconMat = pod.getReconstructionCoeffs();
    printMat(mode1reconMat);

    // quindi, se costruisco il primo modo, posso fare due cose
    // proiettarlo
    // ricostruirlo
    // ho due risultati diversi

    // ora voglio vedere i prodotti L2 dei modi
    // possiamo definire il prodotto L2

    // estrazione matrice autovalori
    std::cout << "matrice degli autovalori" << std::endl;
    ...
    /*
     * matrice con i prodotti scalari dei modi
     */

    // facciamolo per i modi di velocità
    std::vector<std::vector<double>> prodMat;
    prodMat.resize(N_modes, std::vector<double>(N_modes, 0.0));

    for (int i=0; i<N_modes; i++) {
        std::vector<std::vector<double>> modeMat;
        modeMat.resize(N_fields, std::vector<double>(N_modes, 0.0));
        modeMat[2][i] = 1;
        pod::PODField mode_i;
        pod.buildFieldsWithCoeff(modeMat, mode_i);
        std::vector<std::vector<double>> projMat = pod.projectField(mode_i);
        //printMat(projMat);
        for (int j=0; j<N_modes; j++) {
            prodMat[i][j] = projMat[2][j];
        }
    }
    ...
    prodMatP.resize(N_modes, std::vector<double>(N_modes, 0.0));
    ...
    prodMatT.resize(N_modes, std::vector<double>(N_modes, 0.0));
